...
contract AccountsQuery {
  constructor(address[] memory accounts) {
    AccountDescription[] memory descriptions = new AccountDescription[](accounts.length);
    for (uint256 i = 0; i < accounts.length; i++) {
      descriptions[i] = _describeAccount(accounts[i]);
    }
    bytes memory data = abi.encode(descriptions);
    assembly {
      return(add(data, 32), mload(data))
    }
  }

  function describeAccounts(address[] memory account) external view returns (AccountDescription[] memory) {
    AccountDescription[] memory descriptions = new AccountDescription[](account.length);
    for (uint256 i = 0; i < account.length; i++) {
      descriptions[i] = _describeAccount(account[i]);
    }
    return descriptions;
  }
