...
describe("Test", () => {
  ...
  before(async () => {
    ...
    underlying = await ethers.getContractAt("IERC20", await vaultContract.asset());
  });
  ...
  it("Should have right info", async () => {
    ...
    expect(await vaultContract.maxTotalSupply()).to.eq(vault.maxTotalSupply.raw);
    expect(await vaultContract.annualInterestBips()).to.eq(vault.annualInterestBips);
    ...
    expect(await vaultContract.liquidityCoverageRatio()).to.eq(vault.liquidityCoverageBips);
